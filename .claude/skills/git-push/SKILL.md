---
name: git-push
description: Commits authorized pending work via git-commit, syncs with origin (fetch, merge-oriented pull), and pushes the current branch. Watches GitHub Actions until green only when `.github/workflows/` exist; when no CI is configured (Wordbreak default), stop after a successful push. Use when the user asks to push, sync and push, or ship the branch. Requires explicit invocation or a parent workflow that authorizes push. Do not use without permission to write to git remote.
---
...
Close the loop from **authorized local changes → pushed branch → green GitHub
Actions**: commit the scoped work authorized for this push, preserve unrelated
dirty files, integrate upstream with **merge** (not silent rebase unless the
user prefers otherwise), push, watch workflows, and iterate on real failures
until passes.
...
- You need the full “commit if needed → sync → push → watch CI” routine.
...
### 4. Monitor CI (only if it exists)

- If the repo has **no** `.github/workflows/` (Wordbreak default), **stop after
  a successful push**. Say clearly: **no CI configured** — do not invent
  workflows or hunt for Actions runs.
- If workflows exist: identify the run (`gh run list` / `gh run watch`), wait
  for finish, then on failure fix root cause, commit via **`git-commit`**,
  push, and re-watch until green.
- **Never** satisfy a quality gate by lowering thresholds.
...
- **`git-commit`**: message shape and default **scoped** staging (unless a
  parent workflow or explicit user request broadens the scope).
